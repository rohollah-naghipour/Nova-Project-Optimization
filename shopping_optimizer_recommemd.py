...
# ============================================
# 3. تابع بهینه‌سازی با خروجی انگلیسی و خرید جزئی
# ============================================
...
def optimize_shopping_budget(df, budget, required_categories, output_file='shopping_list.txt'):
    ...
    # تعریف تابع هدف به صورت جداگانه
    ...
    for idx, row in df.iterrows():
        ...
        # اضافه کردن به تابع هدف
        objective += buy_vars[var_name] * row['quantity']
    
    # تنظیم تابع هدف یک‌بار
    ...
    # قید حداقل مقدار برای دسته‌های اجباری
    ...
    result = {
        ...
        'total_quantity': 0,
        'items': [],
        'is_feasible': prob.status == 1
    }
    
    # حتی اگر مسئله بهینه نباشد، بهترین جواب ممکن را استخراج می‌کنیم
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        # بررسی اینکه آیا همهٔ نیازمندی‌ها تأمین شده‌اند
        missing_categories = []
        for category, min_qty in required_categories.items():
            purchased = sum([item['total_qty'] for item in result['items'] if item['category'] == category])
            if purchased < min_qty:
                missing_categories.append(f"{category_translation.get(category, category)} (need: {min_qty}, got: {purchased})")
        
        if not result['is_feasible'] or missing_categories:
            f.write("⚠️ WARNING: Budget was NOT sufficient for all requirements!\n")
            f.write("   The system bought the maximum possible with your budget.\n")
            if missing_categories:
                f.write("\n❌ Missing Requirements (could not be fully met):\n")
        ...
        for category, min_qty in required_categories.items():
            ...
            purchased = sum([item['total_qty'] for item in result['items'] if item['category'] == category])
            status = "✅" if purchased >= min_qty else "❌"
            f.write(f"  {status} {cat_en}: required {min_qty} units, purchased {purchased:.0f} units\n")
        f.write("\n")
...
if __name__ == "__main__":
    ...
    if not required:
        ...
        if not result['is_feasible']:
            print("\n⚠️ بودجه برای تأمین همهٔ نیازمندی‌ها کافی نبود!")
            print("   بهترین ترکیب ممکن با بودجهٔ شما خریداری شد.")
            print("   برای مشاهدهٔ جزئیات، فایل خروجی را بررسی کنید.")
