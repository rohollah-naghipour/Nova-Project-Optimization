...
def main():
    if not DATASETS_DIR.exists():
        print(f"❌ پوشه {DATASETS_DIR} پیدا نشد.")
        print(f"   مسیر فعلی: {DATASETS_DIR.resolve()}")
        return
    ...
    if not json_files:
        print(f"⚠️ هیچ فایل JSON توی {DATASETS_DIR} پیدا نشد.")
        return

    print(f"🔍 {len(json_files)} فایل JSON پیدا شد.")
    print("🔌 اتصال به MariaDB ...")
    ...
    try:
        ...
        print(f"\n📊 آمار دیتابیس {DB_CONFIG['database']}:")
